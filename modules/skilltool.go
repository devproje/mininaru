...
func bundleListing(dir string) string {
	...
	for _, entry = range entries {
		if entry.Name() == SKILL_FILE || strings.HasPrefix(entry.Name(), ".") {
			continue
		}
...
func bundlePath(entry *Skill, rel string) (string, error) {
	...
	return readPath(entry.Path, rel)
}
...
func SkillResult(name, rel string) (string, error) {
	var entry *Skill
...
func SkillLoad() Def {
	return Def{
		...
		Execute: func(ctx context.Context, arguments string) (string, error) {
			...
			return SkillResult(strings.TrimSpace(payload.Name), strings.TrimSpace(payload.Path))
		},
	}
}
