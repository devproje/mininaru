...
const CreateToolName = "skill_create"
...
func skillWrite(root, name string, document []byte) (string, error) {
	...
	target = filepath.Join(bundle, skillFile)
...
func CreateResult(name, description, body, scope string, overwrite bool) (string, error) {
	var root string
	var document []byte
	var target string
	...
	root, scope, err = CreateRoot(strings.TrimSpace(scope))
	if err != nil {
		return "", err
	}
...
func CreateTool() modules.Tool {
	return modules.Tool{
		...
		Execute: func(ctx context.Context, arguments string) (string, error) {
			...
			return CreateResult(payload.Name, payload.Description, payload.Body, payload.Scope, payload.Overwrite)
		},
	}
}
